...
// build/server/chunks/_server.ts-Cnnd4F0g.js
...
var GET = async ({ url }) => {
  ...
  const validUrls = [];
  const validSkus = [1];
  const checkResults = await Promise.all(candidates.map(async (url2) => {
    if (await headExists(url2)) return url2;
    return null;
  }));
  for (const url2 of checkResults) if (url2) {
    const proxyUrl = url2.replace("https://img.bnk48cdn.net/shop/", "/api/image/");
    validUrls.push(proxyUrl);
    const match = url2.match(/(\d+)\.\w+$/);
    if (match) {

...
var MAX_ARRAY_LEN, MAX_ARRAY_INDEX, object_proto_names, index_of, includes, object_prototype, array_prototype, has_own_property, noop, DESTROYING, EFFECT_TRANSPARENT, EFFECT_PRESERVED, USER_EFFECT, REACTION_IS_UPDATING, ERROR_VALUE, STALE_REACTION, ATTR_REGEX, CONTENT_REGEX, replacements, whitespace, subscriber_queue, flags;
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    let visibleEnd = 100;
    $$renderer2.push(`<div class="page-shell"><div class="co-page-hero"><div class="co-page-hero__main"><span class="mono-label">Visual infrastructure</span> <h1 class="hero-display">Asset registry</h1> <p class="body-large">Technical repository for indexing and discovering official
    ...
    $$renderer2.select({ value: sortOrder }, ($$renderer3) => {
      $$renderer3.option({ value: "desc" }, ($$renderer4) => {
        $$renderer4.push(`Latest archives`);
      });
      $$renderer3.option({ value: "asc" }, ($$renderer4) => {
        $$renderer4.push(`Historical first`);
      });
    });
    $$renderer2.push(`</div> <div class="search-box jump-box"><input type="number" placeholder="Jump to node ID"${attr("value", jumpToId)}/> <button type="button" class="jump-trigger" aria-label="Jump to ID"><i class="fa-solid fa-arrow-right"></i></button></div></div> `);
    $$renderer2.push("<!--[-1-->");
    $$renderer2.push(`<!--]--> <div class="asset-grid">`);
    $$renderer2.push("<!--[-1-->");
    $$renderer2.push(`<!--]--> <!--[-->`);
    const each_array = ensure_array_like(assets);
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let asset = each_array[$$index];
      $$renderer2.push(`<div class="editorial-card" role="button" tabindex="0"><div class="media-wrap"><img${attr("src", assetThumbSrc(asset))}${attr("alt", asset.title ?? asset.id)} loading="lazy" onload="this.__e=event" onerror="this.__e=event"/> <div class="node-id">#${escape_html(asset.id)}</div></div> <div class="card-overlay"><i class="fa-solid fa-expand"></i></div></div>`);
    }
    $$renderer2.push(`<!--]--> `);
    if (visibleEnd < assets.length) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<svg xmlns="http://www.w3.org/2000/svg" width="100%"${attr("height", (assets.length - visibleEnd) * 2)} aria-hidden="true" role="presentation"></svg>`);
    } else $$renderer2.push("<!--[-1-->");
    $$renderer2.push(`<!--]--></div> <div class="load-more-container"><button class="button-secondary load-more-container"${attr("disabled", loading, true)}>${escape_html(loadMoreLabel())}</button></div></div> `);
    $$renderer2.push("<!--[-1-->");
    $$renderer2.push(`<!--]-->`);
  });
}
...
var index = 5;
var component_cache;
var component = async () => component_cache ??= (await Promise.resolve().then(() => (init_page_svelte_BINRs8_V(), page_svelte_BINRs8_V_exports))).default;
var imports = ["_app/immutable/nodes/5.Cq5Kggod.js", "_app/immutable/chunks/DhkMvD30.js", "_app/immutable/chunks/DUoziJeh.js", "_app/immutable/chunks/CP97kCR3.js", "_app/immutable/chunks/D72LGVUQ.js", "_app/immutable/chunks/D1h2OxpE.js"];
var stylesheets = [];
var fonts = [];
export {
  component,

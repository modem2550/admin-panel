...
// build/server/chunks/_server.ts-DFuRYTlK.js
...
var GET = async ({ url }) => {
  ...
  if (type === "product") {
    const cached2 = cache.get(type);
    if (cached2 && cached2.expires > Date.now()) return json(cached2.data);
    let low2 = 0;
    let high2 = 15e3;
    let lastFoundId2 = 0;
    while (low2 <= high2) {
    ...
    let checkId2 = lastFoundId2 + 1;
    let gapLimit2 = 3;
    while (gapLimit2 > 0 && checkId2 <= 16e3) {
      if (await shopProductExists(checkId2)) {
        lastFoundId2 = checkId2;
        gapLimit2 = 3;
      } else gapLimit2--;
      checkId2++;
    }
    if (lastFoundId2 > 0) {
      let thumbUrl = "";
      try {
        const r = await fetch(`https://public.bnk48.io/shop/product/${lastFoundId2}`);
        if (r.ok) {
  ...
  async function quickProbe(id) {
    ...
    if (type === "group") quickUrls = [`https://img.bnk48cdn.net/shop/product-group/${idStr}.jpg`, `https://img.bnk48cdn.net/shop/product-group/${idStr}.png`];
      ...
      const fallbackUrl = `https://img.bnk48cdn.net/shop/${getDefaultAssetUrl("product", id).replace("/api/image/", "")}`;

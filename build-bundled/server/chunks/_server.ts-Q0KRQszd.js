  ...
  "node_modules/@supabase/auth-js/dist/main/lib/fetch.js"(exports) {
    ...
    async function handleError2(error2) {
      ...
      if (!errorCode) {
        ...
        throw new errors_1.AuthWeakPasswordError(_getErrorMessage2(data), error2.status, ((_a = data.weak_password) === null || _a === void 0 ? void 0 : _a.reasons) || []);
...
var handleError = async (error2, reject, options, namespace) => {
...
// build/server/chunks/supabase.server-DQGJHM27.js
var dummy = new Proxy({}, {
  get: (_target, prop) => {
...
function getAdminClient() {
  const supabaseUrl = private_env.VITE_SUPABASE_URL;
  const serviceKey = private_env.SUPABASE_SERVICE_ROLE_KEY;
  const anonKey = private_env.VITE_SUPABASE_ANON_KEY;
  ...
  if (!_adminClient) {
    if (!serviceKey) console.warn("[supabase.server] SUPABASE_SERVICE_ROLE_KEY missing \u2014 falling back to anon key (RLS applies)");
    _adminClient = createClient(supabaseUrl, key, { auth: {
...
// build/server/chunks/_server.ts-Q0KRQszd.js
var PERFORMANCE_LIST_URL = "https://public.bnk48.io/performance/list";
var PERFORMANCE_URL = "https://public.bnk48.io/performance/";
var HISTORICAL_ID_MIN = 14;
var DEFAULT_MAX_BUFFER = 10;
var MEMBER_CACHE_TTL_MS = 3600 * 1e3;
...
var cachedAllIds = null;
var cacheExpiresAt = 0;
var CACHE_TTL_MS = 300 * 1e3;
async function getAllPerformanceIds() {
  if (cachedAllIds && Date.now() < cacheExpiresAt) return cachedAllIds;
  const idSet = /* @__PURE__ */ new Set();
  let maxIdFound = HISTORICAL_ID_MIN;
  try {
    const { data } = await supabaseAdmin.from("cdn_assets").select("id").eq("type", "archive").order("id", { ascending: false }).limit(1).maybeSingle();
  ...
  try {
    ...
    if (resp.ok) {
      ...
      const items = [...Array.isArray(data.available) ? data.available : [], ...Array.isArray(data.upcoming) ? data.upcoming : []];
      for (const item of items) if (typeof item.eventId === "number") {
  ...
  for (let id = searchLimit; id >= HISTORICAL_ID_MIN; id--) idSet.add(id);
...
var GET = async ({ url }) => {
  ...
  try {
    ...
    if (url.searchParams.get("raw") === "1") return json({
      total: allIds.length,
      ids: pageIds
    });
    const validPairs = (await Promise.all(pageIds.map((id) => fetchPerformance(id)))).map((item, idx) => ({
      item,
      id: pageIds[idx]
    })).filter(({ item }) => item !== null);
    const assets = await Promise.all(validPairs.map(async ({ item, id }) => {
      const memberIds = Array.isArray(item.memberIdList) ? item.memberIdList : [];
      const memberNames = memberIds.length > 0 ? await Promise.all(memberIds.map((mid) => getMemberName(mid))) : [];
    ...
    if (syncRows.length > 0) supabaseAdmin.from("cdn_assets").upsert(syncRows, { onConflict: "id,type" }).then(({ error: dbErr }) => {
      if (!dbErr) return;

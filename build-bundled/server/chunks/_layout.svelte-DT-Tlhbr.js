...
function writable(value, start = noop) {
  ...
  function subscribe2(run2, invalidate = noop) {
    ...
    if (subscribers.size === 1) stop = start(set2, update2) || noop;
    run2(value);
    return () => {
...
function render(component, options2 = {}) {
  ...
  return Renderer.render(component, options2);
}
...
var Renderer = class Renderer2 {
  ...
  */
  static render(component, options2 = {}) {
    ...
    Object.defineProperties(result, {
      html: { get: () => {
        return (sync ??= Renderer2.#render(component, options2)).body;
      } },
      head: { get: () => {
        return (sync ??= Renderer2.#render(component, options2)).head;
      } },
      body: { get: () => {
        return (sync ??= Renderer2.#render(component, options2)).body;
      } },
      ...
      then: { value: (onfulfilled, onrejected) => {
        {
          const result2 = sync ??= Renderer2.#render(component, options2);
  ...
  */
  static #render(component, options2) {
    ...
    try {
      const renderer = Renderer2.#open_render("sync", component, options2);
  ...
  */
  static async #render_async(component, options2) {
    const previous_context = ssr_context;
    try {
      const renderer = Renderer2.#open_render("async", component, options2);
...
function _mount(Component, { target, anchor, props = {}, events, context, intro = true, transformError }) {
  ...
  mounted_components.set(component, unmount3);
  return component;
}
...
var index_server_exports = __exportAll({
  afterUpdate: () => noop,
  beforeUpdate: () => noop,
...
function Toast($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    var $$store_subs;
    $$renderer2.push(`<div class="notification-stack"><!--[-->`);
    const each_array = ensure_array_like(store_get($$store_subs ??= {}, "$toasts", toasts));
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      let toast = each_array[$$index];
      $$renderer2.push(`<div${attr_class(`notification-node ${stringify(toast.type)}`)} role="alert"><header class="node-header"><span class="mono-label">${escape_html(toast.type?.toUpperCase() || "SYSTEM_LOG")}</span> <button class="node-close" aria-label="Dismiss"><i class="fa-solid fa-xmark"></i></button></header> <div class="node-body">${escape_html(toast.message)}</div></div>`);
    }
    $$renderer2.push(`<!--]--></div>`);

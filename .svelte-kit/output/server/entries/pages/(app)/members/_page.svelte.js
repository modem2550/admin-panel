	...
	$$renderer.component(($$renderer) => {
		...
		if (filteredMembers().length === 0) {
			...
			for (let $$index_2 = 0, $$length = each_array_2.length; $$index_2 < $$length; $$index_2++) {
				...
				$$renderer.push(`<!--]--></td><td><h3 class="data-table__title">${escape_html(member.name)}</h3></td><td><span${attr_class(`data-table__chip ${stringify(member.graduated_at ? "data-table__chip--muted" : "data-table__chip--live")}`)}>${escape_html(member.graduated_at ? "Alumni" : "Active")}</span></td><td class="data-table__meta"><span>${escape_html(member.brand)}</span> <span class="sep">/</span> <span>${escape_html(member.gen)}</span> <span class="sep">/</span> <span>${escape_html(member.team)}</span></td><td class="data-table__actions"><div class="data-table__ops"><button type="button" class="action-icon-btn" aria-label="Edit entry"><i class="fa-solid fa-pen-to-square"></i></button> <button type="button" class="action-icon-btn danger" aria-label="Delete entry"><i class="fa-solid fa-trash"></i></button></div></td></tr>`);
			}

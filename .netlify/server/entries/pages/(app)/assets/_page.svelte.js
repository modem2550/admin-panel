...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		async function loadNextBatch() {
			...
			scanningStatus = sortOrder === "asc" ? `กำลังโหลด ID ${start}–${start + itemsPerPage - 1} ...` : `กำลังโหลด ID ${start}–${rangeEnd} (ใหม่ไปเก่า) ...`;
			try {
				let resp;
				...
				if (newAssets.length === 0) scanningStatus = `ไม่พบ asset ในช่วงนี้ ลองเปลี่ยนประเภทหรือลำดับ`;
					...
					if (assetType !== "archive") if (sortOrder === "asc") currentCursor = start + itemsPerPage;
					else currentCursor = Math.min(...newAssets.map((a) => parseInt(a.id))) - 1;
					if (assetType === "archive" && archiveTotal !== null) scanningStatus = assets.length >= archiveTotal ? `โหลดครบแล้ว ${assets.length} รายการ` : `โหลดแล้ว ${assets.length} / ${archiveTotal} รายการ`;
					else scanningStatus = "";
				}
			} catch (error) {
				console.error(error);
		...
		function assetThumbSrc(asset) {
			if (assetType !== "product" && assetType !== "archive") return asset.url;
